...
class AptLeadsSetData extends Apt_DB {
...
//function
    ...
    public function email_send($mail_list = array()) {
        ...
        if (!$value) {
            if (empty($mail_list)) {
                $multiemail = get_option('appointment-email');
                $explode_emails = explode(", ", $multiemail);
            } else {
                $explode_emails = $mail_list;
            }
            $email_message = __("Form details below.", 'appointment') . "\n\n";
...
// create email headers
            for ($i = 0; $i < count($explode_emails); $i++) {
                ...
                $email_subject = __("Message Received from a New Lead", 'appointment');
...
//                        'X-Mailer: PHP/' . phpversion();
            ...
            $full_path = '//' . $leadsname . $leaduri;
            echo "<div class='sucess-send'><a href=" . $full_path . ">" . __('Back', 'appointment') . "</a><h2>" . __('Your Message has been successfully submitted and mail sent.', 'appointment') . "</h2></div>";
        }
    }
