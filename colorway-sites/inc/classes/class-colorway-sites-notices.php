<?php
/**
 * Colorway Sites Notices
 *
 * @package Colorway Sites
 * @since 1.0.8
 */

if ( ! class_exists( 'Colorway_Sites_Notices' ) ) :

	/**
	 * Colorway_Sites_Notices
	 *
	 * @since 1.0.8
	 */
	class Colorway_Sites_Notices {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_colorway-notices', array( $this, 'dismiss' ) );
		...
		function enqueue_scripts() {
			wp_register_script( 'colorway-sites-notices', COLORWAY_SITES_URI . 'inc/assets/js/colorway-sites-notices.js', array( 'jquery' ), COLORWAY_SITES_VER, true );
		}
		...
		function show_notices() {
			...
			foreach ( self::$notices as $key => $notice ) {
				...
				$classes = array( 'colorway-notice', 'notice' );
				...
				// Notice ID.
				$notice_id = 'colorway-sites-notices-id-' . $key;
		...
		public static function markup( $notice = array() ) {
			...
			wp_enqueue_script( 'colorway-sites-notices' );
	 ...
	 */
	Colorway_Sites_Notices::get_instance();
