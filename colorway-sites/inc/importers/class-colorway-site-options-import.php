 ...
 * @since  1.0.0
 */
class Colorway_Site_Options_Import {

	/**
	 * Instance of Colorway_Site_Options_Importer
	 *
	 * @since  1.0.0
	 * @var (Object) Colorway_Site_Options_Importer
	 */
	private static $_instance = null;

	/**
	 * Instanciate Colorway_Site_Options_Importer
	 *
	 * @since  1.0.0
	 * @return (Object) Colorway_Site_Options_Importer
	 */
		...
		if ( isset( $cats ) ) {
			...
			foreach ( $cats as $key => $cat ) {
				...
				if ( ! empty( $cat['slug'] ) && ! empty( $cat['thumbnail_src'] ) ) {
					...
					$image = (object) Colorway_Sites_Helper::_sideload_image( $cat['thumbnail_src'] );
	...
	private function insert_logo( $image_url = '' ) {
		...
		$data = (object) Colorway_Sites_Helper::_sideload_image( $image_url );
