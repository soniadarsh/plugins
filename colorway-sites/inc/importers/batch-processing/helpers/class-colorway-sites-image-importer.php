...
if ( ! class_exists( 'Colorway_Sites_Image_Importer' ) ) :

	/**
	 * Colorway Sites Image Importer
	 *
	 * @since 1.0.14
	 */
	class Colorway_Sites_Image_Importer {
		...
		private function get_saved_image( $attachment ) {
			...
			if ( apply_filters( 'colorway_sites_image_importer_skip_image', false, $attachment ) ) {

				Colorway_Sites_Image_Importer::log( 'Download (✕) Replace (✕) - ' . $attachment['url'] );
			...
			$post_id = $wpdb->get_var(
				$wpdb->prepare(
					'SELECT `post_id` FROM `' . $wpdb->postmeta . '`
						WHERE `meta_key` = \'_colorway_sites_image_hash\'
							AND `meta_value` = %s
					;',
		...
		public function import( $attachment ) {
			...
			if ( empty( $file_content ) ) {
				...
				Colorway_Sites_Image_Importer::log( 'Download (✕) Replace (✕) - ' . $attachment['url'] );
				Colorway_Sites_Image_Importer::log( 'Error: Failed wp_remote_retrieve_body().' );
			...
			update_post_meta( $post_id, '_colorway_sites_image_hash', $this->get_hash_image( $attachment['url'] ) );
	 ...
	 */
	Colorway_Sites_Image_Importer::get_instance();
