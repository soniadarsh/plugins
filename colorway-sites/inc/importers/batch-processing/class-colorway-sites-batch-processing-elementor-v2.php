...
/**
 *  Colorway Source Remote
 */
class Colorway_Sites_Batch_Processing_Elementor extends Source_Base {
	...
	public function get_title() {
		return __( 'Remote', 'colorway-sites' );
	}
	...
	public function process_export_import_content( $content, $method ) {
		return Plugin::$instance->db->iterate_data(
			$content, function( $element_data ) use ( $method ) {
				...
				$instance = new Colorway_Sites_Batch_Processing_Elementor();
	...
	public function import() {
		...
		\Colorway_Sites_Image_Importer::log( '---- Processing WordPress Posts / Pages - for Elementor ----' );
	...
	public function import_single_post( $post_id = 0 ) {
		...
		if ( ! empty( $post_id ) ) {
			...
			$hotlink_imported = get_post_meta( $post_id, '_colorway_sites_hotlink_imported', true );
			...
			if ( empty( $hotlink_imported ) ) {
				...
				if ( ! empty( $data ) ) {
					...
					update_metadata( 'post', $post_id, '_colorway_sites_hotlink_imported', true );
